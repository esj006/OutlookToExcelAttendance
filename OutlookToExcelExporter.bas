Attribute VB_Name = "OutlookToExcelExporter"
Option Explicit
...
End If
    ...
    If olAppointment.Subject = meetingTitle Then
        ...
        ' Sjekk for distribusjonslister først
        Dim distributionListFound As Boolean
        Dim distributionListName As String
        distributionListFound = False
        For Each olRecipient In olAppointment.recipients
            ...
            If olRecipient.AddressEntry.GetExchangeUser Is Nothing And _
               Not olRecipient.AddressEntry.GetExchangeDistributionList Is Nothing Then
                distributionListFound = True
                distributionListName = olRecipient.AddressEntry.GetExchangeDistributionList.Name
                Exit For
            End If
        Next olRecipient

        If distributionListFound Then
            MsgBox "A distribution list named '" & distributionListName & "' has been detected in the meeting. " & _
               "To ensure all members of this list are included in the Excel sheet, please open the meeting in Outlook, navigate to 'Tracking', and expand the distribution list '" & distributionListName & "'. Run the code again." & vbNewLine & vbNewLine & _
               "Please note that the response status for individual members of a distribution list may not be accurately reflected in Excel/Outlook. This is due to Outlook's treatment of distribution lists as a single entity, hence individual response statuses for members within the list may not always be precisely trackable." & vbNewLine & vbNewLine & _
               "For optimal tracking of individual responses in Outlook, it is advisable to either invite participants directly or expand the distribution list before sending the invitation", _
               vbExclamation, "Distribution List Detected"

            Exit Sub
        End If

        ' Sjekk om dataene er uendret
